...
if [[ "$PIPELINE_EXIT" -ne 0 ]]; then
    if echo "$PIPELINE_STDERR" | grep -qiE "does not exist|not supported|not a chat model|model_not_found|not found"; then
        skip "claude-sonnet-5: model not available"
    fi
fi
...
assert_file_exists "$LOGS_DIR/run/utils.py" "utils.py written to disk"
assert_file_exists "$LOGS_DIR/run/main.py" "main.py written to disk"

output="$(cat "$LOGS_DIR/run/tool_output.txt")"
assert_contains "$output" "Hello, Attractor!" "output contains Hello, Attractor!"

pass "claude-sonnet-5: Python multi-file write generated and validated"
