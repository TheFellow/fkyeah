...
if [[ -n "${MOCK_LLM_SERVER:-}" && -x "$MOCK_LLM_SERVER" ]]; then
  "$MOCK_LLM_SERVER" --port "$PORT" --record-dir "$SERVER_DIR" --scenario edge_override >"$SERVER_LOG" 2>&1 &
else
  dotnet run --project "$TEST_DIR/../../../tests/Fixtures/MockLlmServer/MockLlmServer.fsproj" -- \
    --port "$PORT" --record-dir "$SERVER_DIR" --scenario edge_override >"$SERVER_LOG" 2>&1 &
fi
SERVER_PID=$!
