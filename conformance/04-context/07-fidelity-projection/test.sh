...
# The edge gen->sink has fidelity="truncate" (default budget ~= 400 chars).
...
# Verify no value in context.json exceeds 400 chars (truncation applied)
...
if [ "$max_val_len" -gt 400 ]; then
    fail "A3 fidelity projection: context value length $max_val_len exceeds truncation limit 400"
fi
