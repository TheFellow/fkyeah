module McpClient.SseParsingTests
...
let ``parseSseStream raises TimeoutException on complete silence`` () =
    task {
        ...
        let events =
            Transport.parseSseStreamWithIdleTimeout (tp.ReadStream()) CancellationToken.None 300

        let sw = Stopwatch.StartNew()
        let! ex = Assert.ThrowsAsync<TimeoutException>(fun () -> (drainAsync events) :> Task)
...
let ``parseSseStream outer cancellation exits cleanly without exception`` () =
    task {
        ...
        let events =
            Transport.parseSseStreamWithIdleTimeout (tp.ReadStream()) outer.Token 5000
        // McpClient convention: outer cancellation exits the loop cooperatively
        // (no exception), matching existing Transport behavior.
        let! collected = drainAsync events
