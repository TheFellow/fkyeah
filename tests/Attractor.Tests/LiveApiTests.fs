...
module SmokeTest =
    let private runTest (provider: string) =
        ...
        let logsDir = Path.Combine(Path.GetTempPath(), "attractor-live-test-" + Guid.NewGuid().ToString("N").[..7])
        Directory.CreateDirectory(logsDir) |> ignore
        try
            let registry = HandlerRegistry.CreateDefault(llmClient = client)
            let config = { RunConfig.Default(logsDir) with Registry = registry }
            ...
            try Directory.Delete(logsDir, true) with _ -> ()
