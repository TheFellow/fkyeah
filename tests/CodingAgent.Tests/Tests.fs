...
let makeSimpleMock () =
    let mock = ConfigurableMockAdapter("test")
    mock.SetCompleteHandler(fun req ->
        { Id = "r1"; Model = req.Model; Provider = "test"
...
let makeToolMock () =
    ...
    mock.SetCompleteHandler(fun _req ->
        ...
        if callCount = 1 then
            let tc = { Id = "call_1"; Name = "read_file"; Arguments = """{"file_path":"/tmp/test.txt"}"""; Metadata = Map.empty }
            { Id = "r1"; Model = "m"; Provider = "test"
              Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
              FinishReason = ToolCalls "tool_calls"; Usage = { InputTokens = 10; OutputTokens = 5; ReasoningTokens = None; CacheReadTokens = None; CacheWriteTokens = None }
            ...
            { Id = "r2"; Model = "m"; Provider = "test"
              ...
              FinishReason = Stop "stop"; Usage = { InputTokens = 20; OutputTokens = 10; ReasoningTokens = None; CacheReadTokens = None; CacheWriteTokens = None }
              ResponseId = None; Raw = None; Warnings = []; RateLimit = None })
    mock

/// Create a temp directory for tests
let createTempDir () =
    let dir = Path.Combine(Path.GetTempPath(), "coding-agent-test-" + Guid.NewGuid().ToString("N").[..7])
...
type TestProfile(model: string) =
    interface IProviderProfile with
        ...
        member _.ToolDefinitions = [
            SharedTools.readFile; SharedTools.writeFile; SharedTools.editFile
            SharedTools.shell; SharedTools.grep; SharedTools.glob
        ]
        member _.BuildSystemPrompt(env, projectDocs, userInstructions) =
            let parts = [
                ...
                match projectDocs with Some d -> d | None -> ""
                match userInstructions with Some u -> u | None -> ""
            ]
...
module CoreLoopTests =
    ...
    let ``Session turn limits max_turns stops loop`` () =
        ...
        try
            ...
            let client = Client()
            client.RegisterAdapter(makeSimpleMock())
            let config = { SessionConfig.Default with MaxTurns = 2 }
            let session = Session(TestProfile("m"), env, client, config)
    ...
    let ``Multiple sequential inputs work`` () =
        ...
        try
            ...
            Assert.Equal(Idle, session.State)
            let userTurns = session.History |> List.filter (fun t -> match t with UserTurn _ -> true | _ -> false)
        ...
        try
            ...
            mock.SetCompleteHandler(fun _req ->
                ...
                if callCount <= 12 then
                    let tc = { Id = sprintf "call_%d" callCount; Name = "shell"; Arguments = """{"command":"echo same"}"""; Metadata = Map.empty }
                    { Id = "r"; Model = "m"; Provider = "test"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                else
                    { Id = "done"; Model = "m"; Provider = "test"
                      Message = Message.assistant("Done")
                      FinishReason = Stop "stop"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None })
            let client = Client()
            client.RegisterAdapter(mock)
...
module ProviderProfileTests =
    ...
    let ``Each profile produces provider-specific system prompt`` () =
        ...
        try
            let env = LocalExecutionEnvironment(dir) :> IExecutionEnvironment
            let openai = (OpenAIProfile("gpt-5.2") :> IProviderProfile).BuildSystemPrompt(env, None, None)
            let anthropic = (AnthropicProfile("claude-opus-4-6") :> IProviderProfile).BuildSystemPrompt(env, None, None)
            let gemini = (GeminiProfile("gemini-3-flash-preview") :> IProviderProfile).BuildSystemPrompt(env, None, None)
    ...
    let ``Custom tools can be registered on top of profile`` () =
        ...
        let customTool = { Name = "custom_tool"; Description = "Custom"; Parameters = """{"type":"object"}""" }
        profile.AddCustomTool(customTool)
        let names = (profile :> IProviderProfile).ToolDefinitions |> List.map (fun t -> t.Name)
    ...
    let ``Tool name collisions resolved by custom overriding profile default`` () =
        let profile = AnthropicProfile("claude-opus-4-6")
        let customShell = { Name = "shell"; Description = "Custom shell override"; Parameters = """{"type":"object"}""" }
...
module ToolExecutionTests =
    ...
    let ``Unknown tool calls return error result not exception`` () =
        ...
        try
            let env = LocalExecutionEnvironment(dir) :> IExecutionEnvironment
            let registry = AgentToolRegistry()
            let tc = { Id = "c1"; Name = "unknown_tool"; Arguments = """{}"""; Metadata = Map.empty }
            let result = registry.Dispatch(tc, env)
        ...
        try
            ...
            registry.Register({ Definition = SharedTools.shell
                                Execute = fun args _ ->
                                    receivedArgs <- args
                                    "ok" })
            let tc = { Id = "c1"; Name = "shell"; Arguments = """{"command":"ls -la"}"""; Metadata = Map.empty }
...
module TruncationTests =
    ...
    let ``Line-based truncation runs second`` () =
        ...
        let config = { SessionConfig.Default with ToolOutputLimits = Map.ofList [ "shell", 1000000 ] }
...
module SteeringTests =
    ...
    let ``steer queues message injected after current tool round`` () =
        ...
        try
            ...
            let steeringTurns = session.History |> List.filter (fun t ->
                match t with SteeringTurn _ -> true | _ -> false)
    ...
    let ``follow_up queues message processed after current input completes`` () =
        ...
        try
            ...
            let userTurns = session.History |> List.filter (fun t ->
                match t with UserTurn _ -> true | _ -> false)
    ...
    let ``Steering messages appear as SteeringTurn in history`` () =
        ...
        try
            ...
            let steering = session.History |> List.choose (fun t ->
                match t with SteeringTurn(content, _) -> Some content | _ -> None)
...
module ReasoningEffortTests =
    ...
    let ``reasoning_effort is passed through to LLM request`` () =
        ...
        try
            ...
            mock.SetCompleteHandler(fun req ->
                ...
                { Id = "r"; Model = req.Model; Provider = "test"
                  Message = Message.assistant("ok")
                  FinishReason = Stop "stop"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None })
            let client = Client()
            client.RegisterAdapter(mock)
            let config = { SessionConfig.Default with ReasoningEffort = Some "high" }
            let session = Session(TestProfile("m"), env, client, config)
    ...
    let ``Valid reasoning effort values`` () =
        ...
        for value in [ "low"; "medium"; "high" ] do
            let config = { SessionConfig.Default with ReasoningEffort = Some value }
            Assert.Equal(Some value, config.ReasoningEffort)
        let defaultConfig = { SessionConfig.Default with ReasoningEffort = None }
    ...
    let ``Changing reasoning_effort mid-session takes effect`` () =
        ...
        let config1 = { SessionConfig.Default with ReasoningEffort = Some "low" }
        let config2 = { config1 with ReasoningEffort = Some "high" }
...
module SystemPromptTests =
    ...
    let ``System prompt includes provider-specific base instructions`` () =
        ...
        try
            let env = LocalExecutionEnvironment(dir) :> IExecutionEnvironment
            let prompt = (AnthropicProfile("claude-opus-4-6") :> IProviderProfile).BuildSystemPrompt(env, None, None)
    ...
    let ``User instruction overrides appended last`` () =
        ...
        try
            let env = LocalExecutionEnvironment(dir) :> IExecutionEnvironment
            let prompt = (AnthropicProfile("claude-opus-4-6") :> IProviderProfile)
...
module HistoryConverterTests =
    ...
    let ``AssistantTurn converts to assistant message with tool calls`` () =
        let tc = { Id = "c1"; Name = "shell"; Arguments = """{}"""; Metadata = Map.empty }
    ...
    let ``ToolResultsTurn converts to tool result messages`` () =
        let results = [
            { ToolCallId = "c1"; Content = "output"; IsError = false; ImageData = None; ImageMediaType = None }
        ]
...
module LoopDetectionTests =
    ...
    let ``No loop detected with varied tool calls`` () =
        let turns: Turn list = [
            for i in 1..10 ->
                AssistantTurn("", [ { Id = sprintf "c%d" i; Name = sprintf "tool%d" i; Arguments = "{}"; Metadata = Map.empty } ], None, Usage.Zero, DateTime.UtcNow)
        ]
    ...
    let ``Loop detected with identical repeating calls`` () =
        let turns: Turn list = [
            for _ in 1..10 ->
                AssistantTurn("", [ { Id = "c1"; Name = "shell"; Arguments = """{"command":"echo same"}"""; Metadata = Map.empty } ], None, Usage.Zero, DateTime.UtcNow)
        ]
    ...
    let ``Short history does not trigger loop`` () =
        let turns: Turn list = [
            AssistantTurn("", [ { Id = "c1"; Name = "shell"; Arguments = """{}"""; Metadata = Map.empty } ], None, Usage.Zero, DateTime.UtcNow)
        ]
        Assert.False(LoopDetection.detectLoop turns 10)
    ...
    let ``Session auto-registers write_file without manual wiring`` () =
        ...
        try
            ...
            mock.SetCompleteHandler(fun _ ->
                callCount <- callCount + 1
                if callCount = 1 then
                    let tc = { Id = "call_1"; Name = "write_file"; Arguments = """{"file_path":"auto-registered.txt","content":"ok"}"""; Metadata = Map.empty }
                    { Id = "r1"; Model = "m"; Provider = "test"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                else
...
module Sprint002EventTests =
    ...
    let ``Streaming mode emits incremental AssistantTextDelta events`` () =
        ...
        try
            ...
            let deltas =
                session.Events
                |> List.choose (fun e ->
                    if e.Kind = AssistantTextDelta then e.Data |> Map.tryFind "delta"
                    else None)
    ...
    let ``Context window warning fires at eighty percent threshold`` () =
        ...
        try
            ...
            let warning =
                session.Events
                |> List.tryFind (fun e -> e.Kind = Warning)
...
module Sprint002PromptTests =
    ...
    let ``Environment context reports non-git directories`` () =
        ...
        try
            let env = LocalExecutionEnvironment(dir) :> IExecutionEnvironment
            let prompt = (OpenAIProfile("gpt-5.2") :> IProviderProfile).BuildSystemPrompt(env, None, None)
    ...
    let ``Environment context includes git branch for git repositories`` () =
        ...
        try
            let env = LocalExecutionEnvironment(dir) :> IExecutionEnvironment
            env.ExecCommand("git init -q && git config user.email test@example.com && git config user.name test && echo hi > a.txt && git add a.txt && git commit -m init -q", 10000, None) |> ignore
            let prompt = (OpenAIProfile("gpt-5.2") :> IProviderProfile).BuildSystemPrompt(env, None, None)
    ...
    let ``ExecCommand timeout sends TERM before kill on unix-like platforms`` () =
        if OperatingSystem.IsWindows() then
            ...
            try
                ...
                Assert.True(result.DurationMs >= 1900, $"Expected SIGTERM grace window before SIGKILL, got {result.DurationMs}ms")
            finally cleanupDir dir
    ...
    let ``Shell timeout output includes retry guidance message`` () =
        ...
        try
            ...
            mock.SetCompleteHandler(fun _ ->
                callCount <- callCount + 1
                if callCount = 1 then
                    let tc = { Id = "call_1"; Name = "shell"; Arguments = """{"command":"sleep 5","timeout_ms":50}"""; Metadata = Map.empty }
                    { Id = "r1"; Model = "m"; Provider = "test"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                else
            ...
            let toolOutput =
                ...
                |> List.tryPick (fun t ->
                    ...
                    | ToolResultsTurn(results, _) ->
                        results |> List.tryHead |> Option.map (fun r -> r.Content)
                    | _ -> None)
                |> Option.defaultValue ""
    ...
    let ``Gemini read_many_files and list_dir executors work`` () =
        ...
        try
            ...
            mock.SetCompleteHandler(fun _ ->
                ...
                | 1 ->
                    let tc = { Id = "call_1"; Name = "read_many_files"; Arguments = """{"paths":["a.txt","b.txt"]}"""; Metadata = Map.empty }
                    { Id = "r1"; Model = "m"; Provider = "gemini"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | 2 ->
                    let tc = { Id = "call_2"; Name = "list_dir"; Arguments = """{"path":"."}"""; Metadata = Map.empty }
                    { Id = "r2"; Model = "m"; Provider = "gemini"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | _ ->
                    { Id = "r3"; Model = "m"; Provider = "gemini"
                      Message = Message.assistant("done")
                      FinishReason = Stop "stop"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None })
...
module Sprint002SubagentToolTests =
    ...
    let ``Subagent tools spawn send wait close execute end-to-end`` () =
        ...
        try
            ...
            mock.SetCompleteHandler(fun req ->
                ...
                match callCount with
                | 1 ->
                    let tc = { Id = "call_spawn"; Name = "spawn_agent"; Arguments = """{"task":"subtask"}"""; Metadata = Map.empty }
                    { Id = "r1"; Model = req.Model; Provider = "openai"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | 2 ->
                    { Id = "sub_1"; Model = req.Model; Provider = "openai"
                ...
                | 3 ->
                    ...
                    let tc = { Id = "call_send"; Name = "send_input"; Arguments = $"""{{"agent_id":"{agentId}","message":"continue"}}"""; Metadata = Map.empty }
                    { Id = "r3"; Model = req.Model; Provider = "openai"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | 4 ->
                    { Id = "sub_2"; Model = req.Model; Provider = "openai"
                      Message = Message.assistant("subagent follow-up")
                      FinishReason = Stop "stop"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | 5 ->
                    let agentId = lastAgentId |> Option.defaultValue ""
                    let tc = { Id = "call_wait"; Name = "wait"; Arguments = $"""{{"agent_id":"{agentId}"}}"""; Metadata = Map.empty }
                    { Id = "r5"; Model = req.Model; Provider = "openai"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | 6 ->
                    let agentId = lastAgentId |> Option.defaultValue ""
                    let tc = { Id = "call_close"; Name = "close_agent"; Arguments = $"""{{"agent_id":"{agentId}"}}"""; Metadata = Map.empty }
                    { Id = "r6"; Model = req.Model; Provider = "openai"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | _ ->
                    { Id = "r_final"; Model = req.Model; Provider = "openai"
                      Message = Message.assistant("done")
                      FinishReason = Stop "stop"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None })
    ...
    let ``Subagent depth limit returns tool error result`` () =
        ...
        try
            ...
            let client = Client()
            client.RegisterAdapter(mock)
            let config = { SessionConfig.Default with MaxSubagentDepth = 0 }
    ...
    let ``Subagent working_dir override changes child cwd`` () =
        ...
        try
            ...
            mock.SetCompleteHandler(fun req ->
                ...
                match callCount with
                | 1 ->
                    let tc = { Id = "call_spawn"; Name = "spawn_agent"; Arguments = """{"task":"write file","working_dir":"child"}"""; Metadata = Map.empty }
                    { Id = "r1"; Model = req.Model; Provider = "openai"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | 2 ->
                    let tc = { Id = "sub_write"; Name = "write_file"; Arguments = """{"file_path":"wd.txt","content":"ok"}"""; Metadata = Map.empty }
                    { Id = "sub1"; Model = req.Model; Provider = "openai"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | 3 ->
                    { Id = "sub2"; Model = req.Model; Provider = "openai"
    ...
    let ``Subagent model override uses requested model`` () =
        ...
        try
            ...
            mock.SetCompleteHandler(fun req ->
                ...
                match callCount with
                | 1 ->
                    let tc = { Id = "call_spawn"; Name = "spawn_agent"; Arguments = """{"task":"hello","model":"gpt-sub-model"}"""; Metadata = Map.empty }
                    { Id = "r1"; Model = req.Model; Provider = "openai"
                      Message = { Role = Assistant; Content = [ ToolCall tc ]; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | 2 ->
                    subModel <- Some req.Model
                    { Id = "sub"; Model = req.Model; Provider = "openai"
                      Message = Message.assistant("sub done")
                      FinishReason = Stop "stop"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                | _ ->
                    { Id = "done"; Model = req.Model; Provider = "openai"
                      Message = Message.assistant("done")
                      FinishReason = Stop "stop"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None })
            let client = Client()
            client.RegisterAdapter(mock)
...
module Sprint004Coverage =
    ...
    let ``ProviderOptions from SessionConfig are forwarded to every LLM request`` () =
        ...
        try
            ...
            let config = { SessionConfig.Default with ProviderOptions = Some providerOptions }
            let session = Session(TestProfile("m"), env, client, config)
            session.ProcessInput("hello")
    ...
    let ``Session uses parallel tool dispatch and preserves result order when supported`` () =
        ...
        try
            ...
            mock.SetCompleteHandler(fun _ ->
                ...
                if callCount = 1 then
                    let tcs =
                        [ { Id = "c1"; Name = "slow_tool"; Arguments = """{"n":1}"""; Metadata = Map.empty }
                          { Id = "c2"; Name = "slow_tool"; Arguments = """{"n":2}"""; Metadata = Map.empty }
                          { Id = "c3"; Name = "slow_tool"; Arguments = """{"n":3}"""; Metadata = Map.empty } ]
                    { Id = "r1"; Model = "m"; Provider = "test"
                      Message = { Role = Assistant; Content = tcs |> List.map ToolCall; Name = None; ToolCallId = None }
                      FinishReason = ToolCalls "tool_calls"; Usage = Usage.Zero; ResponseId = None; Raw = None; Warnings = []; RateLimit = None }
                else
                    { Id = "r2"; Model = "m"; Provider = "test"
                      Message = Message.assistant("done")
