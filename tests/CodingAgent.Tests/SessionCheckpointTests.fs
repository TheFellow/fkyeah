...
let ``session checkpoint save and restore round trips history`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "coding-agent-checkpoint-" + Guid.NewGuid().ToString("N"))
    ...
    try
        ...
        let restored =
            Session.RestoreFromCheckpoint(CodingAgent.Tests.TestProfile("m"), env, client, path)
...
let ``load with missing file returns Error with not found`` () =
    let path =
        Path.Combine(Path.GetTempPath(), "nonexistent-" + Guid.NewGuid().ToString("N") + ".json")
...
let ``load with invalid JSON returns Error`` () =
    let path =
        Path.Combine(Path.GetTempPath(), "invalid-json-" + Guid.NewGuid().ToString("N") + ".json")
...
let ``load with wrong version returns Error with version mismatch`` () =
    let path =
        Path.Combine(Path.GetTempPath(), "wrong-version-" + Guid.NewGuid().ToString("N") + ".json")

    try
        let checkpoint =
            { defaultCheckpoint () with
                Version = 999 }

        let bytes =
            System.Text.Json.JsonSerializer.SerializeToUtf8Bytes(checkpoint, SessionPersistence.jsonOptions)
...
let ``save creates parent directory if needed`` () =
    let root =
        Path.Combine(Path.GetTempPath(), "checkpoint-parent-" + Guid.NewGuid().ToString("N"))
...
let ``round-trip preserves SessionId`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rt-sessionid-" + Guid.NewGuid().ToString("N"))
    ...
    try
        let checkpoint =
            { defaultCheckpoint () with
                SessionId = "my-unique-session-42" }

        Async.RunSynchronously(persistence.Save path checkpoint) |> ignore

        let loaded =
            Async.RunSynchronously(persistence.Load path) |> Result.defaultWith failwith
...
let ``round-trip preserves WorkingDirectory`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rt-workdir-" + Guid.NewGuid().ToString("N"))
    ...
    try
        let checkpoint =
            { defaultCheckpoint () with
                WorkingDirectory = "/some/special/path" }
...
let ``round-trip preserves Model`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rt-model-" + Guid.NewGuid().ToString("N"))
...
let ``round-trip preserves empty steering and followup queues`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rt-empty-queues-" + Guid.NewGuid().ToString("N"))
...
let ``round-trip preserves non-empty steering queue`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rt-steering-" + Guid.NewGuid().ToString("N"))
...
let ``atomic write means file exists after save`` () =
    let dir =
        Path.Combine(Path.GetTempPath(), "rt-atomic-" + Guid.NewGuid().ToString("N"))
...
let ``turnOfDto fails on unknown turn case`` () =
    ...
    let ex =
        Assert.Throws<Exception>(fun () -> SessionPersistence.turnOfDto dto |> ignore)
...
let ``eventKindOfString fails on unknown event kind`` () =
    let ex =
        Assert.Throws<Exception>(fun () -> SessionPersistence.eventKindOfString "MysteryEvent" |> ignore)
