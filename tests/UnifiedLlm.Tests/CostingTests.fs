...
let ``cached cost replay is not billed twice`` () =
    let usage = { Usage.Zero with InputTokens = 10; OutputTokens = 5; CacheReadTokens = Some 10 }
    let breakdown =
...
let ``cost ledger accumulates totals`` () =
    ...
    let usage = { Usage.Zero with InputTokens = 10; OutputTokens = 5 }
...
let ``summary format contains Total and token counts`` () =
    let ledger = CostLedger.inMemory()
    let usage = { Usage.Zero with InputTokens = 100; OutputTokens = 50 }
    let cost = Costing.tryCalculateCostById "gpt-5.4" usage false |> Option.get
    ledger.Record cost
...
let ``ledger thread safety with 100 concurrent records`` () =
    ...
    let tasks =
        [| for _ in 1..100 ->
            System.Threading.Tasks.Task.Run(fun () -> ledger.Record cost) |]
