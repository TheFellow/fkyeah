...
let ``failures below threshold keep circuit Closed`` () =
    let config = { FailureThreshold = 5; CooldownPeriod = TimeSpan.FromSeconds(30.0); ProbeSuccessThreshold = 2 }
...
let ``non-transient failure does not count toward threshold`` () =
    let config = { FailureThreshold = 2; CooldownPeriod = TimeSpan.FromSeconds(30.0); ProbeSuccessThreshold = 2 }
...
let ``check in Open before cooldown returns Error`` () =
    let config = { FailureThreshold = 1; CooldownPeriod = TimeSpan.FromSeconds(60.0); ProbeSuccessThreshold = 2 }
...
let ``thread safety with concurrent RecordFailure posts`` () =
    let config = { FailureThreshold = 200; CooldownPeriod = TimeSpan.FromSeconds(30.0); ProbeSuccessThreshold = 2 }
