...
module SimpleTextGeneration =
    ...
    let private runTest (provider: string) =
        ...
        let result =
            Generation.generate client model (Some "Say hello in one word") None None None 0 (Some provider) None None
...
module MultipleParallelToolCalls =
    ...
    let private runTest (provider: string) =
        ...
        let totalToolCalls = result.Steps |> List.sumBy (fun s -> s.ToolCalls.Length)

        Assert.True(
            totalToolCalls >= 2 || result.Steps.Length >= 2,
            sprintf
                "Expected >= 2 tool calls or >= 2 steps. Got %d tool calls, %d steps"
                totalToolCalls
                result.Steps.Length
        )
    ...
    let private runTest (provider: string) =
        ...
        let result =
            Generation.generate
                ...
                model
                (Some
                    "For London: first get the temperature, then get the humidity, then get the 3-day forecast. Use each tool separately in order.")
                None
                None
                (Some tools)
                5
...
module InvalidApiKeyError =
    ...
    let private runTest (provider: string) =
        ...
        let ex =
            Assert.ThrowsAny<ProviderError>(fun () -> client.Complete(request) |> ignore)
...
module UsageTokenCounts =
    ...
    let private runTest (provider: string) =
        ...
        Assert.True(
            result.Usage.TotalTokens = expected,
...
module PromptCaching =
    ...
    let private longSystemPrompt =
        String.replicate
            200
            "This is a long system prompt designed to test prompt caching behavior across multiple API calls. "
...
module ProviderOptionsPassthrough =
    ...
    let private runTest (provider: string) =
        ...
        let providerOptions =
            match provider with
            | "anthropic" -> Some(Map.ofList [ "anthropic", Map.ofList [ "auto_cache", true :> obj ] :> obj ])
            | "openai" -> Some(Map.ofList [ "openai", Map.ofList [ "store", false :> obj ] :> obj ])
            | "gemini" -> Some(Map.ofList [ "gemini", Map.ofList [ "safetySettings", ([]: obj list) :> obj ] :> obj ])
            | _ -> None
