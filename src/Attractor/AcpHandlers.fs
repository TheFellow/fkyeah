...
module AcpHandlers =
                    ...
                    async {
                        ...
                        if hasNext then
                            ...
                            | Ok(Request request) ->
                                ...
                                | "session/prompt" ->
                                    let parameters = request.Params |> Option.defaultValue (Json.serializeToElement {| |})
                                    let sessionId = Json.tryGetString "sessionId" parameters |> Option.defaultValue "memory-session"
                                    let responseText =
                                        $"[Simulated ACP] Session {sessionId} completed."
                                    let payload =
                                        Json.serializeToElement
                                            {| sessionId = sessionId
    ...
    let private recordDelegateDenial operation (denials: ResizeArray<string>) result =
        match result with
        | Error(AcpError.PermissionDenied message) ->
            denials.Add($"{operation}: {message}")
        | Error(AcpError.PathOutsideRoot message) ->
            denials.Add($"{operation}: {message}")
        | _ -> ()
        result
        ...
        interface IHandler with
            member _.Execute(node, context, graph, logsRoot) =
                ...
                | Ok endpoint ->
                    ...
                    history.Add(box {| kind = "request"; method = "initialize" |})
