...
type IProviderProfile =
    ...
    abstract member BuildSystemPrompt: env: IExecutionEnvironment * projectDocs: string option * userInstructions: string option -> string
...
module SharedTools =
    let readFile = {
        ...
        Parameters = """{"type":"object","properties":{"file_path":{"type":"string","description":"Path to the file, relative to the working directory or absolute"},"offset":{"type":"integer","description":"1-based line number to start reading from"},"limit":{"type":"integer","description":"Max lines to read (default: 2000)"}},"required":["file_path"]}"""
    }

    let writeFile = {
        Name = "write_file"
        Description = "Write content to a file. Creates the file and parent directories if needed."
        Parameters = """{"type":"object","properties":{"file_path":{"type":"string","description":"Path to the file, relative to the working directory or absolute"},"content":{"type":"string","description":"The full file content"}},"required":["file_path","content"]}"""
    }

    let editFile = {
        Name = "edit_file"
        Description = "Replace an exact string occurrence in a file."
        Parameters = """{"type":"object","properties":{"file_path":{"type":"string","description":"Path to the file, relative to the working directory or absolute"},"old_string":{"type":"string","description":"Exact text to find"},"new_string":{"type":"string","description":"Replacement text"},"replace_all":{"type":"boolean","description":"Replace all occurrences (default: false)"}},"required":["file_path","old_string","new_string"]}"""
    }

    let shell = {
        Name = "shell"
        Description = "Execute a shell command. Returns stdout, stderr, and exit code."
        Parameters = """{"type":"object","properties":{"command":{"type":"string","description":"The command to run"},"timeout_ms":{"type":"integer","description":"Override default timeout"},"description":{"type":"string","description":"Human-readable description of what this does"}},"required":["command"]}"""
    }
    ...
    let glob = {
        ...
        Parameters = """{"type":"object","properties":{"pattern":{"type":"string","description":"Glob pattern (e.g., **/*.ts)"},"path":{"type":"string","description":"Base directory"}},"required":["pattern"]}"""
    }

    let readManyFiles = {
        Name = "read_many_files"
        Description = "Read multiple files in one call. Returns each file with line-numbered content."
        Parameters = """{"type":"object","properties":{"paths":{"type":"array","items":{"type":"string"},"description":"List of file paths to read"},"offset":{"type":"integer","description":"1-based line number to start reading from"},"limit":{"type":"integer","description":"Max lines per file (default: 2000)"}},"required":["paths"]}"""
    }

    let listDir = {
        Name = "list_dir"
        Description = "List directory entries with optional recursion depth."
        Parameters = """{"type":"object","properties":{"path":{"type":"string","description":"Directory path to list (default: .)"},"depth":{"type":"integer","description":"Max recursion depth (default: 1)"}},"required":[]}"""
    }

    let applyPatch = {
        Name = "apply_patch"
        Description = "Apply code changes using the patch format (v4a). Supports creating, deleting, and modifying files."
        Parameters = """{"type":"object","properties":{"patch":{"type":"string","description":"The patch content in v4a format"}},"required":["patch"]}"""
    }

    let spawnAgent = {
        Name = "spawn_agent"
        Description = "Spawn a subagent to handle a scoped task autonomously."
        Parameters = """{"type":"object","properties":{"task":{"type":"string","description":"Natural language task description"},"working_dir":{"type":"string","description":"Subdirectory to scope the agent to"},"model":{"type":"string","description":"Model override"},"max_turns":{"type":"integer","description":"Turn limit"}},"required":["task"]}"""
    }

    let sendInput = {
        Name = "send_input"
        Description = "Send a message to a running subagent."
        Parameters = """{"type":"object","properties":{"agent_id":{"type":"string","description":"Subagent ID"},"message":{"type":"string","description":"Message to send"}},"required":["agent_id","message"]}"""
    }

    let wait = {
        Name = "wait"
        Description = "Wait for a subagent to complete and return its result."
        Parameters = """{"type":"object","properties":{"agent_id":{"type":"string","description":"Subagent ID"}},"required":["agent_id"]}"""
    }
...
module EnvironmentContext =
    ...
    let private tryGit (workingDir: string) (args: string) =
        runProcess workingDir "git" args

    let private gitContext (workingDir: string) =
        let isGitRepository =
            if Directory.Exists(Path.Combine(workingDir, ".git")) then true
            else
    ...
    let build (env: IExecutionEnvironment) (model: string) =
        let isGitRepository, branch, modifiedCount, recentCommits = gitContext env.WorkingDirectory
        let commitBlock =
            if recentCommits.IsEmpty then "Recent commits: none available"
            else
                let lines = recentCommits |> List.map (fun c -> sprintf "- %s" c) |> String.concat "\n"
                "Recent commits:\n" + lines
        sprintf """<environment>
Working directory: %s
Is git repository: %s
...
type OpenAIProfile(model: string) =
    ...
    member _.AddCustomTool(tool: ToolDefinition) =
        customTools <- tool :: customTools
    ...
    interface IProviderProfile with
        ...
        member _.ToolDefinitions =
            let baseTools = [
                SharedTools.readFile; SharedTools.writeFile; SharedTools.applyPatch
                SharedTools.shell; SharedTools.grep; SharedTools.glob
                SharedTools.spawnAgent; SharedTools.sendInput; SharedTools.wait; SharedTools.closeAgent
            ]
            // Custom tools override base tools with same name
            let customNames = customTools |> List.map (fun t -> t.Name) |> set
            let filtered = baseTools |> List.filter (fun t -> not (customNames.Contains(t.Name)))
...
type GeminiProfile(model: string) =
    ...
    interface IProviderProfile with
        ...
        member _.ToolDefinitions =
            let baseTools = [
                SharedTools.readFile; SharedTools.readManyFiles; SharedTools.writeFile; SharedTools.editFile
                SharedTools.shell; SharedTools.grep; SharedTools.glob
                SharedTools.listDir
                SharedTools.spawnAgent; SharedTools.sendInput; SharedTools.wait; SharedTools.closeAgent
            ]
